...
function writeUrl(mode){const url=new URL(location.href),params=url.searchParams,item=window.index.find(x=>x.file===$('region').value);params.set('region',item.name);params.set('metric',$('metric').value);params.set('distance',$('distance').value);params.set('threshold',$('threshold').value);params.set('sameTik',$('sameTik').checked?'1':'0');params.delete('tik');for(const tik of [...selectedTiks].sort())params.append('tik',tik);requestedUik?params.set('uik',requestedUik):params.delete('uik');history[mode+'State'](null,'',url)}
function uikHref(row){const url=new URL(location.href);url.searchParams.set('region',window.regionInfo.name);url.searchParams.set('uik',row[7]);return url.pathname+url.search}
...
function renderDetail(){
  ...
  for(const neighbor of neighbors){const button=document.createElement('button');button.type='button';button.textContent=`№${neighbor.row[0]} · ${fmt(neighbor.d)} км · явка ${fmt(neighbor.row[4])}% · ЕР ${fmt(neighbor.row[5])}%`;button.onclick=()=>openUik(neighbor.row[7]);card.append(button);L.polyline([[row[2],row[3]],[neighbor.row[2],neighbor.row[3]]],{color:'#168b6c',weight:2,opacity:.55,dashArray:'5 5'}).addTo(detailLayer);L.circleMarker([neighbor.row[2],neighbor.row[3]],{radius:7,color:'#168b6c',weight:2,fillColor:'#fff',fillOpacity:1}).bindTooltip(`УИК №${neighbor.row[0]} · явка ${fmt(neighbor.row[4])}% · ЕР ${fmt(neighbor.row[5])}%`,{permanent:true,direction:'top',className:'selected-uik-tooltip'}).on('click',()=>openUik(neighbor.row[7])).addTo(detailLayer)}
  ...
  L.circleMarker([row[2],row[3]],{radius:10,color:'#087354',weight:3,fillColor:'#fff',fillOpacity:1}).bindTooltip(`УИК №${row[0]} · явка ${fmt(row[4])}% · ЕР ${fmt(row[5])}%`,{permanent:true,direction:'top',className:'selected-uik-tooltip'}).addTo(detailLayer);
...
function calculate(){
  ...
  const shown=new Set();for(const p of pairs){for(const r of [p.a,p.b]){const k=r[7];if(shown.has(k))continue;shown.add(k);L.circleMarker([r[2],r[3]],{radius:4,color:'#243a43',weight:1,fillColor:'#fff',fillOpacity:1}).bindTooltip(`№${r[0]} · явка ${fmt(r[4])}% · ЕР ${fmt(r[5])}%`,{permanent:true,direction:'top',className:'uik-tooltip'}).bindPopup(`УИК №${r[0]} · ${r[1]}<br>Явка ${fmt(r[4])}% · ЕР ${fmt(r[5])}%<br><a href="${uikHref(r)}" data-uik="${r[7]}">Открыть УИК</a>`).addTo(layer)}}
