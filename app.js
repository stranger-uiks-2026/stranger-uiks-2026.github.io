...
function calculate(){
  ...
  activeRows.forEach((a,i)=>{const ci=Math.floor(a[2]/size),cj=Math.floor(a[3]/size),near=[];
    ...
    for(let di=-latCells;di<=latCells;di++)for(let dj=-lonCells;dj<=lonCells;dj++)for(const j of cells.get(`${ci+di},${cj+dj}`)||[]){if(i===j)continue;const b=activeRows[j];if(same&&a[1]!==b[1])continue;if(metric===8&&a[11]!==b[11])continue;const d=km(a,b);if(d<=maxDist)near.push([j,d])}
  ...
  $('stats').innerHTML=`<strong>${fmt(pairs.length)} пар</strong> выше порога<br>${fmt(shown.size)} УИК на карте · ${fmt(activeRows.length)} в выборке${metric===8?` с результатом кандидата из ${fmt(selectedRows.length)}`:''} · ${fmt(rows.length)} с координатами из ${fmt(window.regionInfo.total)} протоколов региона${metric===8&&!activeRows.length?'<br>В исходном файле нет результатов кандидата для выбранной территории.':''}`;
