...
function writeUrl(mode){const url=new URL(location.href),params=url.searchParams,item=window.index.find(x=>x.file===$('region').value);url.pathname=regionPath(item);params.delete('region');$('region').value==='82.json'&&$('area').value?params.set('area',$('area').value):params.delete('area');params.set('metric',$('metric').value);params.set('distance',$('distance').value);params.set('threshold',$('threshold').value);params.set('sameTik',$('sameTik').checked?'1':'0');params.delete('tik');for(const tik of [...selectedTiks].sort())params.append('tik',tik);requestedUik?params.set('uik',requestedUik):params.delete('uik');history[mode+'State'](null,'',url)}
...
function areaRows(){const area=moscowAreas.find(x=>x.code===$('area').value);return $('region').value==='82.json'&&area?rows.filter(row=>area.tiks.includes(row[1])):rows}
function renderArea(){const enabled=$('region').value==='82.json';$('areaFilter').hidden=!enabled;if(!enabled){$('area').value='';return}const old=requestedArea;$('area').replaceChildren(new Option('Все округа',''),...moscowAreas.map(x=>new Option(`${x.name} · ${x.tiks.length} районов`,x.code)));$('area').value=moscowAreas.some(x=>x.code===old)?old:'';requestedArea=$('area').value}
function renderTiks(){
...
function restoreTiks(){const available=new Set(areaRows().map(row=>row[1]));selectedTiks=new Set(requestedTiks.filter(name=>available.has(name)));renderTiks()}
...
async function init(){try{[window.index,moscowAreas]=await Promise.all([fetch('/index.json').then(r=>r.json()),fetch('/moscow-areas.json').then(r=>r.json())]);$('region').innerHTML=window.index.map(x=>`<option value="${x.file}">${x.name} (${x.mapped.toLocaleString('ru-RU')})</option>`).join('');applyUrl();await choose();writeUrl('replace')}catch(e){$('stats').textContent='Не удалось загрузить данные. Откройте сайт через локальный сервер или GitHub Pages.';console.error(e)}}
